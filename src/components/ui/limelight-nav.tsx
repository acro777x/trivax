        ...
        return (
          <a
            ...
            className={`relative z-20 flex h-full cursor-pointer items-center justify-center ${
              shouldShowLabel ? 'gap-1.5 px-3 py-2 text-xs md:text-sm font-medium' : 'p-5'
            } ${iconContainerClassName || ''}`}
          ...
          >
            ...
            {shouldShowLabel && label && (
              <span
                className={`transition-all duration-150 select-none whitespace-nowrap ${
                  activeIndex === index
                    ? 'opacity-100 text-white font-semibold'
                    : 'opacity-45 text-zinc-400 hover:text-zinc-200'
                }`}
              >
                {label}
              </span>
